...
class Arena {
    ...
    public function tick(): void {
        ...
        foreach ($this->getPlugin()->getFFAArenasConfig()->getAll() as $ffaarena => $arenanames) {
            $this->getPlugin()->getServer()->loadLevel($arenanames["world"]);
            /*foreach ($this->getPlugin()->getServer()->getLevelByName($arenanames["world"])->getPlayers() as $player) {
                $this->getPlugin()->getScoreboardUtil()->setFFAScoreboard($player);
            }*/
        }
            /*foreach ($this->getPlugin()->getServer()->getLevelByName($this->getPlugin()->getConfig()->get("lobby"))->getPlayers() as $players){
                $this->getPlugin()->getScoreboardUtil()->setLobbyScoreboard($players);
            }*/
        ...
        if($this->isStarting()) {
            ...
            $this->broadcastPopup("§aMatch starting in §l§3" . $this->countdown . "§r§a...");
            $this->countdown--;
        }
        if($this->isRunning()) {
            foreach($this->getPlayers() as $player) {
        ...
        foreach ($this->getPlayers() as $player) {
            if ($player->getLevel()->getFolderName() === "Sumo1-Event") {
                $this->getPlugin()->getScoreboardUtil()->setSumoEventScoreboard($player);
            }
        }
    }

    public function setDuelsScoreboards() {
        if ($this->isStarting()) {
            foreach ($this->getPlayers() as $player) {
                $arenaname = $this->getPlugin()->getArenaByPlayer($player)->getName();
                $player1 = $this->getPlugin()->getServer()->getPlayer($this->getPlugin()->getActiveDuels()->getNested("$arenaname.player1"));
    ...
    public function join(Player $player): void {
        if(!$this->isIdle()) {
            $player->sendMessage($this->getPlugin()->getPrefix() . TextFormat::RED . "The duel is currently in progress.");
            return;
        }
        if($this->getPlugin()->getArenaByPlayer($player)) {
            $player->sendMessage($this->getPlugin()->getPrefix() . TextFormat::RED . "You are already in a match.");
            return;
        }
    ...
    public function start(): void {
        foreach($this->getPlayers() as $player) {
            ...
            $player->sendTitle("§aFight!", "", 10, 25, 10);
        }
    ...
    public function saveDuelHistory(Player $winner, Player $loser): void {
        ...
        $winnerInformations = [
            ...
            "my-stats" => [
                "items" => $winnerItems,
                "armor" => $winnerArmor,
                "HP" => intval($winner->getHealth()),
                "hunger points" => intval($winner->getFood()),
                "ping" => $winner->getPing(),
                "total hits" => $this->hits[$winnerName]
            ],
            "his-stats" => [
                "items" => $loserItems,
                "armor" => $loserArmor,
                "HP" => intval($loser->getHealth()),
                "hunger points" => intval($loser->getFood()),
                "ping" => $loser->getPing(),
                "total hits" => $this->hits[$loserName]
            ]
        ];
        $loserInformations = [
            "kit" => $this->getKit(),
    ...
    public function getDuration(): string {
        $res = gmdate("H:i:s", $this->gameTime);
        return $res;
    }
}
